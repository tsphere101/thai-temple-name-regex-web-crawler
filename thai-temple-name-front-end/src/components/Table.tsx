...
const Table: FC = ({}) => {
  ...
  return (
    <div className="mx-auto max-w-[100%] flex rounded-lg overflow-hidden sm h-[500px] md:h-[700px]">
      <div className="flex flex-col flex-[2] bg-gray-300 h-full">
        <div className=" bg-dark-gray-a p-3 flex justify-center items-center text-white text-sm font-semibold md:text-base">
          Province
        </div>
        <div className="text-[16px] md:text-[20px] py-4 px-2 md:px-4 gap-2 flex flex-col justify-between md:py-4 lg:py-8 h-full w-full">
          <div className="flex flex-col justify-start items-center gap-2 h-full">
            {provinces_list.map((p) => (
              <Button
                ...
                } md:h-12 shadow-sm shadow-slate-600 active:ring-2 active:ring-slate-500 transition-all ease-in-out`}
                key={uuid()}
                onClick={() => handleProvinceToggled(p.value)}
              >
                {p.name}
                {splist.includes(p.value) && (
                  <div className="absolute top-0 right-0 rounded-full w-5 h-5 md:w-6 md:h-6 lg:h-7 lg:w-7 translate-x-[50%] -translate-y-[50%] bg-stone-700 text-white text-[16px] flex justify-center items-center text-center ease-in-out transition-all">
          ...
          <div className="flex flex-col justify-center items-center gap-2">
            <a
              ...
              } select-none bg-yellow-100 w-full md:h-12 flex justify-center items-center rounded-md py-2 hover:bg-yellow-200 active:bg-yellow-300 ease-in-out transition-all shadow-sm shadow-slate-600 active:ring-2 active:ring-slate-500 md:text-[20px]`}
            >
              <Download className="mr-2 w-[20px] h-[20px] md:w-[30px] md:h-[30px]" />
            ...
            <a
              ...
              className="bg-yellow-100 w-full md:h-12 select-none flex justify-center items-center rounded-md py-2 hover:bg-yellow-200 active:bg-yellow-300 ease-in-out transition-all shadow-sm shadow-slate-600 active:ring-2 active:ring-slate-500" 
            >
              <Download className="mr-2 w-[20px] h-[20px] md:w-[30px] md:h-[30px]" />
      ...
      <div className="flex flex-col flex-[3] bg-white h-full">
        <div className=" bg-dark-gray-b p-3 flex justify-center items-center text-white text-sm font-semibold md:text-base md:font-bold">
          Temples
        </div>
        <div className="text-xs font-light p-4 md: w-full h-full overflow-y-scroll">
            ...
            temples.map((p) => {
              ...
              return (
                <>
                  <h4
                    key={uuid()}
                    className="underline font-normal text-lg md:text-xl mb-1"
                  >
                    {Eng_To_Thai[p.name]}
                  </h4>
                  {filteredTemples.length > 0 && (
                    <ul key={uuid()}>
                      {filteredTemples.map((t, i) => (
                        <li
                          className="list-disc text-md md:text-base ml-5 md:ml-7"
                          key={uuid()}
                        >
                          {t}
                        </li>
